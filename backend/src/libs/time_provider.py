...
class TimeProvider:
    ...
    def format_datetime_local(dt: Optional[datetime] = None) -> str:
        ...
        """
        if dt is None:
            dt = TimeProvider.local_now()
        elif dt.tzinfo is None:
            dt = dt.replace(tzinfo=timezone.utc).astimezone(TimeProvider.GMT_PLUS_7)
        else:
            dt = dt.astimezone(TimeProvider.GMT_PLUS_7)
    ...
    def parse_to_local(timestamp_str: str) -> datetime:
        ...
        return dt.astimezone(TimeProvider.GMT_PLUS_7)
    ...
    def create_log_message(action: str, index: Optional[str] = None) -> str:
        ...
        """
        time_str = TimeProvider.format_time_for_log()
