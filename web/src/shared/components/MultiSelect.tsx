        ...
        <div className="flex flex-wrap gap-1 items-center flex-1 mr-2">
          {selectedOptions.length === 0 ? (
            ...
            selectedOptions.map((option) => (
              <span
                key={option.value}
                className={`inline-flex items-center px-2 text-sm sm:text-base rounded-md ${option.disabled ? 'bg-gray-100 text-gray-500' : 'bg-blue-100 text-blue-800'}`}
              >
                {option.label}
                {!option.disabled && (
        ...
        <div className="absolute z-[9999] w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-80 overflow-hidden">
          ...
          <div className="max-h-48 overflow-y-auto">
            {filteredOptions.length > 0 ? (
              ...
              <div className="px-3 py-4 text-sm text-gray-500 text-center">No devices found</div>
            )}
          </div>
